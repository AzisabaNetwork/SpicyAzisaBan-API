...
router.post('/changepassword', w(async (req, res) => {
  ...
  // Invalidate all sessions except for the current session
  await sql.execute('DELETE FROM `web_sessions` WHERE `user_id` = ? AND `state` != ?', session.user_id, session.state)
  res.send({ message: 'ok' })
}))
